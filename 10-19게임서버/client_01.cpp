...
void go_chatting(SOCKADDR_IN& client_addr, string *id, string *password, int port_num) {        //챗팅방 입장 함수
    //서버와 커넥트 한다.
    ...
    while (1) {     //클라이언트 별 메세지 보내는 라인.
        ...
        if (text == "/back") {
            ...
            SOCKADDR_IN new_client = createSocket();
            MenuList(new_client, id, password);
            break;
        }
...
void MenuList(SOCKADDR_IN& client_addr, string* id, string* password) {
    ...
    cout << "num " << num << endl;
        ...
        case 1:
            ...
            break;
        case 2:
            go_chatting(client_addr, id, password, 7777);
            break;
        case 3:
        ...
        case 5:
            cnt++;
            if (cnt > 2) {
                cout << "방이 꽉 찼습니다. 기다려주세요.\n";
                MenuList(client_addr, id, password);
                break;
            }
            go_chatting(client_addr, id, password, 6666);
        }
...
SOCKADDR_IN createSocket() {
        ...
        //client_addr.sin_port = htons(port_number); 포트번호는 채팅방에 들어가느냐 게임방에 들어가느냐에 따라 바꿔준다.
...
int main() {
    ...
    bool login = true;
    string id, password;
    SOCKADDR_IN client_addr = createSocket();
