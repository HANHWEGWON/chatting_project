...
bool checkThreeNumber(string &msg, int* memo) {
    ...
    while (stream >> sub) {
        ...
        if (!is_digit(sub)) {
            cout << "숫자입니까? msg 프린트->" << x << endl;
            msg = "숫자가 아닌것이 들어와있습니다. 다시 입력해주세요.";
            return false;
        }else if(x < 1 || x > 9) {
            cout << "1~9사이가 아닌가요? msg 프린트->" << x << endl;
            msg = "범위를 넘어선 숫자가 있습니다. 다시 입력해주세요.";
            return false;
        }else if (memo[x] == 1) {
            cout << "중복입니까? msg 프린트->" << x << endl;
...
void game_manager(int turn) { // 쓰레드로 클라이언트에서 오는 데이터 받아오는 함수
    ...
    while (1) {
        ...
        recv(sck_list[turn].sck, buf, MAX_SIZE, 0);
        ...
        if (strike == 3) {          // 승패가 가려지게 되는 코드
            msg = "******** 축하합니다" + sck_list[turn].user + "님이 승리하셨습니다!";
            send_msg(msg.c_str());
