...
void go_chatting(SOCKADDR_IN& client_addr, int port_num) {     //채팅방 입장 함수
    ...
    while (1) {
        ...
        if (text == "/back") {
            ...
            if (text != "/help" && text != "/color" && text.substr(0, 3) != "/dm") {
        ...
        /*if (text == "dm") {
            string dm;
            pstmt = con->prepareStatement("select send_nickname, text, send_time from dm where recv_nickname = ?");
            pstmt->setString(1, real_nickname);
            pstmt->execute();
            result = pstmt->executeQuery();
...
int chat_recv() {
    char buf[MAX_SIZE] = { };
    string msg;
    //memset(&c, 0, sizeof(struct _Color));
    while (1) {
        ZeroMemory(&buf, MAX_SIZE);
        if (recv(client_sock, buf, MAX_SIZE, 0) > 0) {
...
void modify_user_info(string* modify_id, string* password) {
    ...
    if (check_id == *modify_id && check_password == *password) {  //내정보확인하기@@@@@@@@@@@!@@!!@!@!@!@@!!@!@!@!@!@
        cout << "1.비밀번호  2.닉네임  3.이메일  4.뒤로가기    *5*내정보확인" << endl;
...
int main() {
    PlaySound(L"Lawrence-TrackTribe.wav", 0, SND_FILENAME | SND_ASYNC | SND_LOOP);
    /*CSound::Init();
